...
RSpec.describe QuestionsController, type: :controller do
  ...
  let (:my_question) { Qestion.create(
      id: 1,
  ...
  describe "POST create" do
    it "increases the number of questions by 1" do
      expect {post :create, question: {title: "Title", copy : RandomData.random_paragraph, resolved: false}}.to
      change(Question, :count).by(1)
    end
    it "assigns the new question to @question" do
      post :create, {question: my_question.attributes}
      expect(assigns(assigns(:question)).to eq Question.last
    end
  ...
  describe "PUT update" do
    it "updates question with expected attributes" do
      ...
      expect(updated_question.id).to eq my_questioin.id
